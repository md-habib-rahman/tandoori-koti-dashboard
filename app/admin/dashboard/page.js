...
export default function AdminOverviewPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 py-2">
      {/* Welcome Banner */}
      <div className="bg-gradient-to-r from-amber-700 to-amber-900 rounded-3xl p-8 text-white shadow-xs flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        <div>
          <span className="text-xs uppercase tracking-widest font-semibold text-amber-200 bg-amber-800/60 px-3 py-1 rounded-full border border-amber-600/50">
            Control Center
          </span>
        ...
        <div className="flex gap-3 shrink-0">
          <Link
            ...
            className="px-4 py-2.5 bg-white text-amber-900 text-xs font-bold rounded-xl hover:bg-amber-50 transition-colors shadow-xs"
          >
            + Add Dish
          </Link>
          <Link
            href="/admin/dashboard/buffet"
            className="px-4 py-2.5 bg-amber-800/80 border border-amber-600/60 text-white text-xs font-bold rounded-xl hover:bg-amber-800 transition-colors"
          >
            Schedule Buffet
          </Link>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        <div className="bg-white border border-slate-200/90 rounded-2xl p-6 shadow-xs">
          ...
          <span className="text-[11px] text-amber-700 font-medium mt-1 inline-block">
            Across {stats.categoriesCount} menu categories
          </span>
        </div>
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    {item.imageUrl ? (
                      ...
                      <div className="w-10 h-10 bg-slate-100 rounded-lg flex items-center justify-center text-slate-400 text-xs shrink-0 font-bold">
                        N/A
                      </div>
                    )}
        ...
        <div className="lg:col-span-5 bg-white border border-slate-200/90 rounded-2xl p-6 shadow-xs space-y-4">
          <div className="flex items-center justify-between pb-3 border-b border-slate-100">
            <div>
              ...
              <p className="text-xs text-slate-500">Upcoming lunch dates</p>
            </div>
            <Link
              href="/admin/dashboard/buffet"
              className="text-xs font-semibold text-amber-700 hover:text-amber-800"
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-1">
                    {sched.dishes?.slice(0, 3).map((dish) => (
                      <span
                        key={dish.id}
                        className="text-[10px] bg-white text-slate-700 border border-slate-200 px-2 py-0.5 rounded"
                      >
                        {dish.nameEn}
                      </span>
                    ))}
